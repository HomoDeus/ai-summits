...
- `components/atlas/terrain.tsx`: a shared SVG atlas of all catalog problems, grouped by discipline at fixed coordinates. Pan and zoom change the viewport; filters only dim nonmatching peaks. Every peak has a keyboard-accessible selection button. State colors follow the selected year, and an adjacent summary explains the selected problem. Named mountain height combines the two editorial ratings. Unnamed mountains are unscored background geometry clipped to shared province borders. Human and AI climbers have independent dated evidence; route positions are categorical, not completion scores. `lib/map-model.ts` defines shared borders, height mapping, human history selection and the trilingual map key.
